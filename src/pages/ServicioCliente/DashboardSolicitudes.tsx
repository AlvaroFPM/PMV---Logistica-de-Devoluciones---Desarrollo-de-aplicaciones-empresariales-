    ...
    <div className="max-w-6xl mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`py-3 px-6 text-sm font-medium transition-colors ${pestanaActiva === 'Pendiente' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-t-lg'}`}
        >
        ...
        <button
          ...
          className={`py-3 px-6 text-sm font-medium transition-colors ${pestanaActiva === 'Completado' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-t-lg'}`}
        >
                  ...
                  <tr key={solicitud.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      {pestanaActiva === 'Pendiente' ? (
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${esUrgente ? 'bg-red-50 text-red-700 border-red-200 animate-pulse' : estaAlLimite ? 'bg-orange-50 text-orange-700 border-orange-200' : 'bg-green-50 text-green-700 border-green-200'}`}>
                          {esUrgente ? '🔥 Vence Hoy/Mañana' : estaAlLimite ? '⚠️ Cerca de vencer' : `⏳ Quedan ${diasParaExpirar ?? 0} días`}
                        </span>
                      ) : (
